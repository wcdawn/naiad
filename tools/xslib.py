...
def outscatter(xsmat):
    ngroup = len(xsmat["sigma_t"])
    sigma_tr = xsmat["sigma_t"].copy()
    for g in range(ngroup):
        sigma_tr[g] -= np.sum(xsmat["scatter"][1, :, g])
    return sigma_tr


def inscatter(xsmat, spectrum):
    ngroup = len(xsmat["sigma_t"])
    sigma_tr = xsmat["sigma_t"].copy()
    for g in range(ngroup):
        sigma_tr[g] = (
            xsmat["sigma_t"][g]
            - np.sum(xsmat["scatter"][1, g, :] * spectrum) / spectrum[g]
        )
    return sigma_tr
